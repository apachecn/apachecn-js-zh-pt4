...
![image](img/sq.jpg) **注意** Windows Store 应用程序开发人员有两种方式来访问其应用程序的本地存储文件夹。如前所述，可以使用`Windows.Storage.ApplicationData.current.localFolder`。此外，在 WinJS 应用程序中，您可以使用 WinJS 的等效项`WinJS.Application.local.folder`。
