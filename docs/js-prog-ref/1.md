...
*   *渲染引擎*:负责读取 HTML 文档及其相关资产(比如图像和级联样式表)并决定它们的外观。呈现引擎是 DOM 存在的地方。
*   *网络引擎*:负责接入网络。
...
![9781430246299_Fig01-01.jpg](images/9781430246299_Fig01-01.jpg)
...
![image](images/sq.jpg) **注意**你必须同时使用开始和结束标签。HTML5 标准不允许使用自结束标记(尽管有些浏览器可能允许)。
...
事实证明，web 浏览器有一个显而易见的特定解析顺序:浏览器从顶部开始解析 HTML 文档，然后一路向下，当特定的资产到达浏览器时就加载它们。这意味着一个脚本(无论是内联的还是链接的)只能引用一些东西(样式、其他脚本、HTML 元素等等)。)在文档中位于它之上。
...
*   在 Chrome 中，你可以通过“定制和配置谷歌浏览器”菜单访问控制台。选择工具![images](images/arrow.jpg) JavaScript 控制台。您还会看到用于访问控制台的键盘快捷键(Windows 为 Control-Shift-J)。您可以在此处直接键入代码示例。
*   在 Firefox 中，选择工具![images](images/arrow.jpg) Web Developer ![images](images/arrow.jpg)错误控制台。您可以在标有“代码”的框中键入代码示例。如果要使用控制台，请确保在按 Enter 键之前键入完整的有效语句。要了解 JavaScript 中的语句是由什么组成的，你可以跳到第二章[。](2.html)
...
![9781430246299_Fig01-02.jpg](images/9781430246299_Fig01-02.jpg)
