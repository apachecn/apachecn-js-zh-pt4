...
如果您在此时运行示例应用并导航到`SemanticZoom.html`页面，您将看到如图[图 16-1](#fig_16_1) 所示的应用布局。面板结构和配置控件已经就位，但是左侧面板中没有控件。在接下来的部分中，我将向您展示如何应用`SemanticZoom`控件。
...
如果你现在运行这个例子，你将会在应用布局中看到两个`ListView`控件，如图 16-3 中的[所示。](#fig_16_3)
