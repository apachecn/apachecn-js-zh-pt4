...
Windows 通过发送一个`activated`事件通知你的应用它需要执行一个搜索操作。这与您的应用启动时发送的事件相同，为了区分这两种情况，您必须读取事件的`kind`属性。处理一个契约的激活需要一种不同的方法来实现你的应用中的`onactivated`功能，如清单 21-5 中的[所示。](#list_21_5)
